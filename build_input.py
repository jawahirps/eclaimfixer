#!/usr/bin/env python3
"""Generate script_input.xlsx with DHA eClaim data."""

import openpyxl
from openpyxl.styles import Font, PatternFill, Alignment
from openpyxl.utils import get_column_letter

# Claim data organized by source
CLAIMS_BY_SOURCE = {
    "INS012": [
...
def create_input_file(filename="script_input.xlsx"):
    ...
    for source in ["INS012", "TPA002", "TPA003"]:
        color_fill = PatternFill(start_color=SOURCE_COLORS[source], fill_type="solid")
    ...
    wb.save(filename)
    total_claims = claim_index - 1
    print(f"✓ Created {filename} with {total_claims} claims")
    print(f"  - INS012: {len(CLAIMS_BY_SOURCE['INS012'])} claims")
    print(f"  - TPA002: {len(CLAIMS_BY_SOURCE['TPA002'])} claims")
    print(f"  - TPA003: {len(CLAIMS_BY_SOURCE['TPA003'])} claims")
